...
function renderTeamChips() {
    ...
    INITIAL_TEAMS.forEach(team => {
        ...
        chip.innerHTML = `
            ...
            <span>${team.name}</span>
            ${isBye ? '<span style="font-size:0.75rem; color:var(--bye-text); margin-left:4px;">(부전승)</span>' : ''}
        `;
...
function createMatchCard(match) {
    ...
    card.innerHTML = `
        ...
        <div class="match-actions">
            ${(match.team1 && match.team2) ? `<button class="btn-edit-score" onclick="openScoreModal('${match.id}')">✏️ 결과 입력</button>` : '<span style="font-size:0.78rem; color:var(--text-dim);">대진 대기 중</span>'}
        </div>
    `;
...
function setupEventListeners() {
    ...
    if (shuffleBtn) shuffleBtn.addEventListener('click', shuffleBracket);
    ...
    if (pkCheckbox) {
        pkCheckbox.addEventListener('change', (e) => {
            const detailInput = document.getElementById('modal-pk-detail');
